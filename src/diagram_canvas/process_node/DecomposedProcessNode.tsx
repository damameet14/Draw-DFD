...
export const DecomposedProcessNode = ({ data, selected }: NodeProps<ProcessNodeType>) => {
    ...
    if (entityIds.length > 0) {
        ...
        entityIds.forEach((entityId, index) => {
            ...
            // Named explicitly rather than `data`, which would shadow this
            // component's `data` prop holding the process node itself.
            const entityFlowGroup = entityFlows.get(entityId)!;

            // Incoming (Entity->Process) first, then Outgoing (Process->Entity),
            // so an entity's flows sit adjacent on the circle.
            const entityFlowHandles: { id: string, type: 'source' | 'target', edgeOffset: number }[] = [];

            entityFlowGroup.incoming.forEach(id => {
                const edge = diagram.edges.find(e => e.id === id);
                entityFlowHandles.push({ id, type: 'target', edgeOffset: edge?.targetAngleOffset || 0 });
            });
            entityFlowGroup.outgoing.forEach(id => {
                const edge = diagram.edges.find(e => e.id === id);
                entityFlowHandles.push({ id, type: 'source', edgeOffset: edge?.sourceAngleOffset || 0 });
            });

            // Position flows around center with 5 degree spacing
            const spacing = 5;
            const flowCount = entityFlowHandles.length;
            const startOffset = -((flowCount - 1) * spacing) / 2;

            entityFlowHandles.forEach((flow, flowIdx) => {
    ...
    const onResize = (_event: any, params: any) => {
        ...
        diagram.nodes.filter(n => n.level === data.level && n.type === 'process').length > 1
            ? useDiagramStore.getState().syncProcessNodeDiameter(data.level, newDiameter)
    ...
    return (
        ...
        >
            ...
            {/* Dynamic handles */}
            {areFlowHandlesVisible && handles.map(handle => {
